...
async def websocket_endpoint(websocket: WebSocket):
    global _last_jarvis_response, active_ws
...
def api_list_projects():
    return builder.list_projects()

@app.post("/api/projects/build")
async def api_build_project(data: dict):
    ...
    async def run_build_task():
        try:
            await builder.build_fullstack_project(
                description=description,
                ws=active_ws,
                stack_override=stack if stack else None,
                db_override=db if db else None,
                name_override=name if name else None
            )
        except Exception as e:
            print("Background build error:", e)

    asyncio.create_task(run_build_task())
    return {"status": "started"}

@app.get("/api/projects/{name}/files")
def api_get_project_files(name: str):
    project_path = builder.PROJECTS_DIR / name
    if not project_path.exists():
        raise HTTPException(status_code=404, detail="Project not found")
    
    files = []
    for p in project_path.rglob("*"):
        if p.is_file() and not builder._should_skip(p):
            try:
...
def api_get_file_content(name: str, path: str):
    project_path = builder.PROJECTS_DIR / name
    file_path = project_path / path.lstrip("/")
    if ".." in path or not file_path.exists() or not file_path.is_file():
...
def api_save_file_content(name: str, data: dict):
    path = data.get("path", "")
    content = data.get("content", "")
    project_path = builder.PROJECTS_DIR / name
    file_path = project_path / path.lstrip("/")
    if ".." in path or not file_path.parent.exists():
...
async def api_add_feature(name: str, data: dict):
    feature = data.get("feature", "")
    
    async def run_feature_task():
        try:
            await builder.add_feature(name, feature, ws=active_ws)
        except Exception as e:
            print("Background feature build error:", e)
            
    asyncio.create_task(run_feature_task())
    return {"status": "started"}

@app.post("/api/projects/{name}/fix")
async def api_fix_project(name: str, data: dict):
    error_desc = data.get("error", "")
    
    async def run_fix_task():
        try:
            await builder.fix_project(name, error_desc, ws=active_ws)
        except Exception as e:
            print("Background fix error:", e)
            
    asyncio.create_task(run_fix_task())
    return {"status": "started"}

@app.post("/api/projects/{name}/server/toggle")
async def api_toggle_server(name: str, data: dict):
    ...
    if action == "start":
        stack = builder._detect_stack(project_path)
        url = await builder.start_dev_server(name, project_path, stack, ws=active_ws)
        return {"status": "ok", "url": url}
    else:
        await builder.stop_dev_server(name, ws=active_ws)
        return {"status": "ok"}

@app.post("/api/projects/{name}/deploy")
async def api_deploy_project(name: str):
    async def run_deploy_task():
        try:
            await builder.deploy_to_vercel(name, ws=active_ws)
        except Exception as e:
            print("Background deploy error:", e)
            
    asyncio.create_task(run_deploy_task())
    return {"status": "started"}

@app.post("/api/projects/{name}/open-editor")
def api_open_editor(name: str):
    builder.open_project_in_editor(name)
    return {"status": "ok"}
