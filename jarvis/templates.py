...
"""
    ...
    if stack == "nextjs":
        output_format = """```tsx filename="app/page.tsx"
// COMPLETE Next.js Page with ALL pages/subpages, Three.js 3D, Framer Motion, GSAP
```

```css filename="app/globals.css"
/* Complete Tailwind base + custom glassmorphism utilities */
```

```typescript filename="app/api/data/route.ts"
// Complete Next.js Route Handler for APIs (GET/POST/etc.)
```"""
    elif stack == "express_react":
        output_format = """```tsx filename="client/src/App.tsx"
// COMPLETE React app with ALL pages, Three.js 3D, Framer Motion, GSAP
```

```css filename="client/src/index.css"
/* Complete CSS with custom animations, glassmorphism utilities */
```

```javascript filename="server/index.js"
// Complete Express.js backend with DB & API routes
```"""
    elif stack == "html":
        output_format = """```html filename="index.html"
<!-- Complete HTML5 with embedded custom layouts, 3D Canvas, CDN libraries -->
```

```css filename="style.css"
/* Complete style sheet with custom keyframe animations, styling */
```

```javascript filename="script.js"
// Complete interactive vanilla JS script
```"""
    else:
        output_format = """```tsx filename="frontend/src/App.tsx"
// COMPLETE React app with ALL pages, Three.js 3D, Framer Motion, GSAP
```
...
def get_stack_base_files(stack: str, plan: dict, description: str = "") -> dict:
    if stack == "react_fastapi":
        return get_react_fastapi_files(plan, description)
    elif stack == "nextjs":
        return get_nextjs_base_files(plan, description)
    elif stack == "express_react":
        return get_express_react_base_files(plan, description)
    elif stack == "html":
        return get_html_base_files(plan, description)
    else:
        return get_react_fastapi_files(plan, description)


def get_nextjs_base_files(plan: dict, description: str = "") -> dict:
    title  = plan.get("title", "My App")
    domain = classify_domain(description or plan.get("description", ""), title)
    return {
        "package.json": """{
...
export default nextConfig;""",
        "app/layout.tsx": f"""import React from 'react';
import './globals.css';
export const metadata = {{
  title: "{title}",
  description: "Built by JARVIS",
}};
...
export default function Home() {
  return (
    <div style={{minHeight:'100vh',display:'flex',alignItems:'center',justifyContent:'center',background:'#000',color:'#00e5ff'}}>
      <h1>JARVIS BUILDER IS SCRAPING CODE...</h1>
    </div>
  );
}""",
...
def get_express_react_base_files(plan: dict, description: str = "") -> dict:
    title  = plan.get("title", "My App")
    domain = classify_domain(description or plan.get("description", ""), title)
    return {
        "package.json": """{
...
export default function App() {
  return <div>JARVIS BUILDER IS SCRAPING CODE...</div>;
}""",
...
def get_html_base_files(plan: dict, description: str = "") -> dict:
    title  = plan.get("title", "My App")
    domain = classify_domain(description or plan.get("description", ""), title)
    return {
...
<body>
    <h1>JARVIS HTML App scaffolding...</h1>
</body>
</html>""",
        "style.css": """body { background: #000; color: #00e5ff; }""",
        "script.js": """console.log('JARVIS HTML App initialised');""",
        "_domain": domain
    }
