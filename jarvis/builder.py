...
from spec_engine import SpecEngine, AppSpec
from backend_generator import BackendGenerator
from frontend_generator import FrontendGenerator
from devops_generator import DevOpsGenerator
...
async def generate_project_files(plan: dict, original_description: str, ws=None) -> Dict[str, str]:
    ...
    await _ws_log(ws, f"⚡ Classifying project domain for stack [{stack}]...")

    base_files = templates.get_stack_base_files(stack, plan, original_description)
    domain = base_files.pop("_domain", "generic")
    await _ws_log(ws, f"⚡ Domain: [{domain.upper()}] | AI: {get_active_provider()}")

    # ── Step 0: Spec Engine Integration ──────────────────────────────────────
    try:
        await _ws_log(ws, "📑 Generating structured AppSpec contract via SpecEngine...")
        spec = await SpecEngine().generate(original_description)
        await _ws_log(ws, f"✓ AppSpec created: {spec.name} ({len(spec.models)} models, {len(spec.pages)} pages)")
        
        be_files = BackendGenerator().generate(spec)
        fe_files = FrontendGenerator().generate(spec)
        devops_files = DevOpsGenerator().generate(spec)
        test_files = TestGenerator().generate(spec)

        for k, v in be_files.items():
            base_files[k] = v
        for k, v in fe_files.items():
            base_files[k] = v
        for k, v in devops_files.items():
            base_files[k] = v
        for k, v in test_files.items():
            base_files[k] = v

        await _ws_log(ws, f"✓ SpecEngine injected v2.0 files ({len(be_files)} BE, {len(fe_files)} FE, {len(devops_files)} DevOps, {len(test_files)} Tests)!")
    except Exception as spec_e:
        await _ws_log(ws, f"⚠ SpecEngine notice: {spec_e} — utilizing base template engine")
    ...
    try:
        ...
        try:
            ...
            await _ws_log(ws, f"⚠ AI failed completely: {str(retry_e)[:60]}. Falling back to base templates...")
...
async def install_dependencies(project_path: Path, stack: str, ws=None) -> bool:
    ...
    if stack == "react_fastapi":
        ...
        success = ok1 and ok2
    elif stack == "express_react":
        ok1 = await run_install(["npm", "install"], project_path, "npm install (root)")
        ok2 = await run_install(["npm", "install"], project_path / "client", "npm install (client)")
        ok3 = await run_install(["npm", "install"], project_path / "server", "npm install (server)")
...
async def start_dev_server(project_name: str, project_path: Path, stack: str, ws=None) -> Optional[str]:
    ...
    try:
        if stack == "react_fastapi":
            ...
            fe_path = project_path / "client"
            be_path = project_path / "server"

            be_cmd = ["node", "index.js"]
            fe_cmd = ["npm", "run", "dev"]
            ...
            await _ws_log(ws, f"Started Express backend on port {be_port}")

            # Start frontend
            await _ws_log(ws, f"Starting React frontend on {url}...")
            fe_proc = await asyncio.create_subprocess_exec(
                *fe_cmd, cwd=str(fe_path),
                stdout=asyncio.subprocess.PIPE,
                stderr=asyncio.subprocess.STDOUT,
...
# ─────────────────────────────────────────────
async def build_fullstack_project(description: str, ws=None, stack_override: str = None, db_override: str = None, name_override: str = None) -> dict:
    """
    ...
    try:
        ...
        if stack_override:
            plan["stack"] = stack_override
        if db_override:
            plan["db"] = db_override
        if name_override:
            plan["name"] = name_override
...
# ─────────────────────────────────────────────
# RealAppBuilder Orchestrator (v2.0)
# ─────────────────────────────────────────────
class RealAppBuilder:
    def __init__(self, llm_client=None, workspace_dir: str = "./generated_apps"):
        self.spec_engine = SpecEngine(llm_client)
        self.backend_gen = BackendGenerator()
        self.frontend_gen = FrontendGenerator()
        self.devops_gen = DevOpsGenerator()
        self.test_gen = TestGenerator()
        self.workspace = Path(workspace_dir).resolve()

    async def build(self, prompt: str, app_name: str = None) -> dict:
        spec = await self.spec_engine.generate(prompt)

        backend_files = self.backend_gen.generate(spec)
        frontend_files = self.frontend_gen.generate(spec)
        devops_files = self.devops_gen.generate(spec)
        test_files = self.test_gen.generate(spec)

        app_folder = app_name or spec.name.lower().replace(" ", "_")
        project_dir = (self.workspace / app_folder).resolve()
        project_dir.mkdir(parents=True, exist_ok=True)

        all_files = {}
        all_files.update(backend_files)
        all_files.update(frontend_files)
        all_files.update(devops_files)
